...
for num in [1000,10000]:
    ...
    C=[]
    for t in range(0,int(num/2)):
        sisit = np.array(mtab[0:num-t])*np.array(mtab[t:num])
        C.append(1/(num-t)*np.sum(sisit) - (1/num*np.sum(mtab))**2)
        # if t%1000==0: print(t)

    plt.plot(range(0,int(num/2)),C)
    #plt.semilogy()
    plt.show()
